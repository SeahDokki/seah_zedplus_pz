...
def parse(path):
    ...
    for _ in range(methods):
        name_index, desc_index = struct.unpack_from(">HH", data, pos + 2)
        result.append((pool.get(name_index, "?"), pool.get(desc_index, "?")))
        pos = skip_attributes(pos + 6)
    return result
...
if __name__ == "__main__":
    ...
    for name, descriptor in sorted(parse(path)):
        if not needles or any(n in name.lower() for n in needles):
            print("%-34s %s" % (name, descriptor))
